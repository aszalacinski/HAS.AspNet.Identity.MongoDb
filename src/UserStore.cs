...
namespace Microsoft.AspNetCore.Identity.MongoDb
{
        ...
        where TUser :IdentityUser
    {
        ...
        public static Task<UserStore<TUser>> CreateAsync(IMongoDatabase database) => CreateAsync(database, Constants.DefaultCollectionName);
        public static async Task<UserStore<TUser>> CreateAsync(IMongoDatabase database, string userCollectionName)
        {
            var store = new UserStore<TUser>(database, userCollectionName);
            await store.EnsureIndicesCreatedAsync();
            return store;
        }


        private UserStore(IMongoDatabase database)
            : this(database, Constants.DefaultCollectionName)
        {
        }

        private UserStore(IMongoDatabase database, string userCollectionName)
        {
